use std::env;
use std::fs::File;
use std::io::{Read, Write};
use std::convert::TryInto;

fn defender_ksa() -> Vec<u8> {
    let key: [u8; 256] = [
        
    ];

    let mut sbox: Vec<u8> = (0..=255).collect();
    let mut j: usize = 0;
    for i in 0..256 {
        j = (j + sbox[i] as usize + key[i] as usize) % 256;
        sbox.swap(i, j);
    }
    sbox
}

fn rc4_decrypt(sbox: &mut Vec<u8>, data: Vec<u8>) -> Vec<u8> {
    let mut out = Vec::with_capacity(data.len());
    let mut i: usize = 0;
    let mut j: usize = 0;
    for &byte in data.iter() {
        i = (i + 1) % 256;
        j = (j + sbox[i] as usize) % 256;
        sbox.swap(i, j);
        let val = sbox[(sbox[i] as usize + sbox[j] as usize) % 256];
        out.push(val ^ byte);
    }
    out
}

fn defender_unquarantine(input_file: &str, output_file: &str) -> std::io::Result<()> {
    let mut file = File::open(input_file)?;
    let mut data = Vec::new();
    file.read_to_end(&mut data)?;

    if data.len() < 12 || data[0] != 0x0B || data[1] != 0xad || data[2] != 0x00 {
        return Ok(()); // or handle error
    }

    let mut sbox = defender_ksa();
    let outdata = rc4_decrypt(&mut sbox, data);

    let headerlen = 0x28 + u32::from_le_bytes(outdata[8..12].try_into().unwrap()) as usize;
    let origlen =
        u32::from_le_bytes(outdata[headerlen - 12..headerlen - 8].try_into().unwrap()) as usize;

    if origlen + headerlen == outdata.len() {
        let mut unquar = File::create(output_file)?;
        unquar.write_all(&outdata[headerlen..])?;
    }

    Ok(())
}

fn main() {
    let args: Vec<String> = env::args().collect();

    // Check if there are enough arguments
    if args.len() < 3 {
        eprintln!("Usage: {} <input_file> <output_file>", args[0]);
        return;
    }

    let input_file = &args[1];
    let output_file = &args[2];

    if let Err(e) = defender_unquarantine(input_file, output_file) {
        eprintln!("Error: {}", e);
    }
}
